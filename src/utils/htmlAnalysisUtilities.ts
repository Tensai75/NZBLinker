...
export function analyseText(settings: Settings): {
  ...
  // test for groups
  // any group names
  const groupsRegexpString = `[a-z0-9-_]+(?:[.](?:[a-z0-9-_]+)){2,}`
  const groupsRegexp = new RegExp(groupsRegexpString, 'ig')
  // alt.binaries style group names
  const abGroupsRegexpString = `a(?:lt)?[.]b(?:inaries)?(?:[.](?:[a-z0-9-_]+))+`
  const abGroupsRegexp = new RegExp(abGroupsRegexpString, 'ig')
  // first search for groups with a leading description
  let customGroupsSearchTerms = settings.textSelection.groups.join('|')
  customGroupsSearchTerms = customGroupsSearchTerms != '' ? customGroupsSearchTerms + '|' : ''
  const customGroupsRegexpString = `^(?:(?:${customGroupsSearchTerms}gruppen?|groups?)(?:\\s|:)*)+((?:${groupsRegexpString}[,; ]*)+)$`
  const customGroupsRegexp = new RegExp(customGroupsRegexpString, 'im')
  test = selection.match(customGroupsRegexp)
  if (test) {
    groups = test[1].match(groupsRegexp)?.map((match) => match.trim()) ?? []
  } else {
    // if no groups with a leading description were found, search for a.b. group names in the selection
    test = selection.match(abGroupsRegexp)
    if (test) {
      groups = test.map((match) => match.trim())
    }
  }
  groups = groups.map((group) => group.replace(/^a\.b\./, 'alt.binaries.')) // replace a.b. with alt.binaries.
  ...
  customDateSearchTerms = customDateSearchTerms != '' ? customDateSearchTerms + '|' : ''
  const dateRegexpString = `^(?:(?:${customDateSearchTerms}date|datum)(?:\\s|:)*)+(${normalDateRegexpString}|${timestampRegexpString})$`
  const dateRegexp = new RegExp(dateRegexpString, 'im')
  if (dateRegexp.test(selection)) {
    test = selection.match(dateRegexp)
    if (test) {
      if (isTimestampRegexp.test(test[1])) {
    ...
    // if no date was found, just search for any date or timestamp in the selection
    if (isTimestampRegexp.test(selection)) {
      test = selection.match(isTimestampRegexp)
      if (test) {
        timestamp = new Date(test[1]).getTime() / 1000
        isTimestamp = true
      }
